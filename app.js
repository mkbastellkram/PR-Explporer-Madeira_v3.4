...
const trails = D.trails.map(t => ({...t, routeType:'official_pr', categoryId:'official_pr', baseIconId:'pr_pin'}));
...
function applyVisualSettings(){
  ...
  document.body.dataset.glass = state.settings.glass || settingsDefault.glass;
  document.body.dataset.iconSize = state.settings.iconSize || settingsDefault.iconSize;
  const set = (id,val)=>{const el=$(id); if(el) el.value=val;};
  set('themeSelect', state.settings.theme); set('glassSelect', state.settings.glass); set('iconSizeSelect', state.settings.iconSize); set('profileStyleSelect', state.settings.profileStyle);
...
function trailStatus(t){
  const rec = (D.statuses && D.statuses[t.id]) || null;
  const raw = String(t.status || (rec && rec.status) || rec || '').toLowerCase();
  let key='unknown', label='Status prüfen';
  if(raw.includes('offen') || raw.includes('open')) { key='open'; label='offen'; }
  else if(raw.includes('eingesch') || raw.includes('restricted') || raw.includes('limited') || raw.includes('partially') || raw.includes('teil')) { key='limited'; label='eingeschränkt'; }
  else if(raw.includes('gesperrt') || raw.includes('closed')) { key='closed'; label='gesperrt'; }
  return {key, label, note:(rec&&rec.note)||'', source:(rec&&rec.source)||''};
}
function statusHtml(t){ const st=trailStatus(t); return `<span class="status-badge ${st.key}" title="${esc(st.note||'')}"><i></i>${esc(st.label)}</span>`; }
function statusSourceHtml(){ const m=D.statusMeta||{}; return `<div class="status-source">Status: ${esc(m.sourceLabel||'lokale Statusdatei')} · Stand ${esc(m.lastUpdated||'unbekannt')} · vor Wanderung offizielle Quelle prüfen.</div>`; }
function isCollected(id, kind='trail'){ return state.travel.some(e=>e.id===id && e.kind===kind && !state.hiddenTravel.includes(`${kind}:${id}`)); }
...
function initMap(){
  ...
  overlays.hiking = L.tileLayer('https://tile.waymarkedtrails.org/hiking/{z}/{x}/{y}.png',{maxZoom:18, opacity:.72, pane:'overlayPane'});
...
function applyFilters(){
  ...
  filtered = trails.filter(t => {
    const lev=normLevel(t.level); const hay=[t.number,t.id,t.name,t.region,t.level].join(' ').toLowerCase(); const st=trailStatus(t).key;
    return (state.level==='all'||lev===state.level) && (state.statusFilter==='all'||st===state.statusFilter) && (!q||hay.includes(q)) && (!state.onlyFav||state.favs.has(t.id)) && (!state.onlyCollected||isCollected(t.id,'trail')) && (state.routeType==='all'||state.routeType==='official_pr'||state.routeType==='collected');
  });
...
function endpointIcon(label,cls=''){ return L.divIcon({className:'',html:`<div class="endpoint-marker ${cls}">${label}</div>`,iconSize:[28,28],iconAnchor:[14,14]}); }
function renderEndpoints(id){
  layers.endpoints.clearLayers(); if(!id) return; const tr=D.tracks[id]||[], dr=D.drives[id]||[], t=byId[id];
  if(tr.length){ L.marker(tr[0],{icon:endpointIcon('S'),zIndexOffset:900}).bindPopup('<b>Start Wanderung</b>').addTo(layers.endpoints); L.marker(tr[tr.length-1],{icon:endpointIcon('Z','end'),zIndexOffset:900}).bindPopup('<b>Ziel Wanderung</b>').addTo(layers.endpoints); }
  if(dr.length) L.marker(dr[dr.length-1],{icon:endpointIcon('P','drive'),zIndexOffset:850}).bindPopup('<b>Anfahrt / Parkplatz</b>').addTo(layers.endpoints);
  if(t && !tr.length && !dr.length) L.marker([t.lat,t.lon],{icon:endpointIcon('PR'),zIndexOffset:850}).addTo(layers.endpoints);
}
function renderMap(){
  if(!map) return; layers.pins.clearLayers(); layers.tracks.clearLayers(); layers.drives.clearLayers(); layers.endpoints.clearLayers(); layers.highlight.clearLayers(); syncOverlayLayers(); markers={};
  const showList = filtered;
  if(state.layers.pins){ showList.forEach(t=>{ const m=L.marker([t.lat,t.lon],{icon:markerIcon(t),title:`${t.number} ${t.name}`,zIndexOffset:(state.selectedTrailId===t.id?2200:1200)}).on('click',()=>selectTrail(t.id,{fromMap:true})).addTo(layers.pins); m.bindPopup(`<b>${esc(t.number)} · ${esc(t.name)}</b><br>${esc(t.region)}<br>${fmtKm(t.distanceKm)} · ${fmtMin(t.driveMin)}`); markers[t.id]=m; }); }
  if(state.selectedTrailId){ const id=state.selectedTrailId, w=Number(state.settings.lineWidth)||5, op=(Number(state.settings.lineOpacity)||92)/100; if(state.layers.tracks) lineCased(layers.tracks,D.tracks[id],getComputedStyle(document.body).getPropertyValue('--gpx').trim()||'#ff3b30',state.solo?w+1:w,op); if(state.layers.drives) lineCased(layers.drives,D.drives[id],getComputedStyle(document.body).getPropertyValue('--kml').trim()||'#007aff',state.solo?w+1:w,Math.min(op,.86)); renderEndpoints(id); }
  else if(state.layers.tracks){ filtered.forEach(t=>{ if(D.tracks[t.id]) L.polyline(D.tracks[t.id],{color:'#ff3b30',weight:2,opacity:.24,lineCap:'round'}).addTo(layers.tracks); }); }
...
function bind(){
  ...
  document.querySelectorAll('[data-open-layers]').forEach(b=>b.onclick=()=>{$('layerPanel').classList.remove('hidden');}); document.querySelectorAll('[data-fit-selected]').forEach(b=>b.onclick=()=>state.selectedTrailId?centerTrail(state.selectedTrailId,true):fitAll()); document.querySelectorAll('[data-reset-view]').forEach(b=>b.onclick=()=>{state.solo=false;fitAll();renderMap();}); document.querySelectorAll('[data-clear-selection]').forEach(b=>b.onclick=()=>{state.selectedTrailId=null;state.solo=false;clearContext();hideCarousel();closeSheet();renderMap();});
  ['themeSelect','glassSelect','iconSizeSelect','profileStyleSelect'].forEach(id=>$(id).onchange=e=>{const k={themeSelect:'theme',glassSelect:'glass',iconSizeSelect:'iconSize',profileStyleSelect:'profileStyle'}[id]; state.settings[k]=e.target.value; applyVisualSettings(); renderSheet();});
  ['lineWidth','outlineWidth','lineOpacity'].forEach(id=>$(id).oninput=e=>{const k={lineWidth:'lineWidth',outlineWidth:'outlineWidth',lineOpacity:'lineOpacity'}[id]; state.settings[k]=+e.target.value; saveState(); renderMap();});
  $('resetVisuals').onclick=()=>{state.settings={...settingsDefault}; applyVisualSettings(); renderMap(); renderSheet(); toast('Darstellung zurückgesetzt');};
  bindSheetDrag(); bindCarouselDrag(); window.addEventListener('resize',()=>setTimeout(()=>map?.invalidateSize(),150)); window.addEventListener('orientationchange',()=>setTimeout(()=>map?.invalidateSize(),350));
}
...
function boot(){
  ...
  renderTravel();
  updateControls();
  /* V3.4.9 Cache-Bypass: service worker registration disabled for test */
